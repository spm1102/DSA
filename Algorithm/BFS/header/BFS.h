...
static void BFS(graph_t* p_graph, unsigned int startVertex);
...
inline void BFS(graph_t* p_graph, unsigned int startVertex) {
    ...
    for(unsigned int i = 0; i < p_graph->numVertices; i++){
        if(i != startVertex) {
            printf("Shortest path from %d to %d: ",startVertex, i);
            print_parentPath(parentVertex, i, startVertex);
            printf("\n");
        }
    }
    QUEUE_Free(p_queue);
